...
import com.youllbecold.trustme.ui.viewmodels.NotificationAction
import com.youllbecold.trustme.ui.viewmodels.NotificationViewModel
...
private fun NotificationScreen(
    ...
    val notificationPermState = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        rememberPermissionState(PermissionHelper.notificationPermission)
    } else {
        null // Not needed on lower APIs
    }

    val bgLocationPermissionState = if (askForBgLocation) {
        rememberPermissionState(PermissionHelper.bgLocationPermission)
    } else {
        ...
        action = {
            when {
                !notificationGranted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU ->
                    notificationPermLauncher.launch(PermissionHelper.notificationPermission)
